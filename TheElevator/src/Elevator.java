...
public class Elevator extends JPanel implements ActionListener, Serializable,
	...
	public Elevator(JButton swingAct, JButton[] swingButtons) {
		...
		initGUI(swingAct, swingButtons);
	}

	public void initGUI(JButton swingAct, JButton[] swingButtons) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (floor < 1 || floor > 3)
			throw new RuntimeException(
					"Error - Elevator moved to bad floor number: " + floor);
		setFloor(floor);
		repaint();
		resetButton();
	...
	public void setFloor(int f) {
		...
		// Problem 5:
		direction[1] = (!floorBits[1] && floorBits[0] && !buttons[0])
				|| (floorBits[1] && !floorBits[0] && !buttons[1])
				|| (floorBits[0] && floorBits[1] && !buttons[2]);

		direction[0] = (!floorBits[1] && buttons[2] && !buttons[0] && floorBits[0])
				|| (!buttons[0] && floorBits[0] && !floorBits[1] && buttons[1])
				|| (floorBits[1] && !floorBits[0] && buttons[2] && !buttons[1] && !buttons[0])
				|| (!floorBits[1] && !buttons[0] && !buttons[1] && buttons[2]);

		// Problem 6:
		lightUp = (!floorBits[0] && floorBits[1])
				|| (floorBits[0] && !floorBits[1] && !buttons[0]);
		lightDown = (floorBits[0])
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < 3; i++) {
			...
			Rectangle2D.Double elevator = new Rectangle2D.Double(
					7 * width / 16, i * height / 3 + height / 16, width / 8,
					7 * height / 32);
			if (floor == 3 - i) {
				if (doorOpen) {
					...
					g2.setColor(Color.BLACK);
					g2.draw(elevator);
					g2.fillRect((int) elevator.getX(), (int) elevator.getY(),
							width / 32, 7 * height / 32);
					g2.fillRect((int) (elevator.getX() + elevator.getWidth())
							- width / 32, (int) elevator.getY(), width / 32,
							7 * height / 32);
				} else {
					g2.fill(elevator);
				}
				...
				if (direction[0] == true && direction[1] == true) {
					g2.fillRect(5 * width / 16, (int) elevator.getY() + height
							/ 32 - 2, width / 32, 5 * height / 32 + 4);
					Polygon arrow = new Polygon();
					arrow.addPoint(5 * width / 16 + width / 64,
							(int) elevator.getY() - height / 64);
					arrow.addPoint(5 * width / 16 - 3 * width / 64,
							(int) elevator.getY() + height / 32);
					arrow.addPoint(5 * width / 16 + 5 * width / 64,
							(int) elevator.getY() + height / 32);
					g2.fill(arrow);
					...
					Polygon arrow = new Polygon();
					arrow.addPoint(5 * width / 16 + width / 64,
							(int) elevator.getY() + 15 * height / 64);
					arrow.addPoint(5 * width / 16 - 3 * width / 64,
							(int) elevator.getY() + 6 * height / 32);
					arrow.addPoint(5 * width / 16 + 5 * width / 64,
							(int) elevator.getY() + 6 * height / 32);
					g2.fill(arrow);
				} else if (direction[0] == false && direction[1] == false) {
					g2.fillOval(4 * width / 16, (int) elevator.getY() + 3
							* height / 32 - 2, 3 * width / 32,
							2 * height / 32 + 4);
				}
	...
	public static void saveState(Elevator e, String filename) {
		...
		accessor.writeObject(filename, e);
	}
	
	public static Elevator loadState(String filename) {
		FileIO accessor = new FileIO();
		return (Elevator)accessor.readObject(filename);
	}
	...
	public void windowClosing(WindowEvent e) {
		Elevator.saveState(this, "gg.elv");
	}
